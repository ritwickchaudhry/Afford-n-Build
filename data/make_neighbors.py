...
from data.get_corners import get_corners

def get_bounds(corners, dim):
...
def make_neighbors(all_corners, areas, extents, orient_deg=10, trans_amt=5, overlap_thresh=0.5):
	...
	for corners in all_corners:
		min_x = min(get_bounds(corners, 0))
		max_x = max(get_bounds(corners, 0))
		min_y =  min(get_bounds(corners, 1))
		max_y =  max(get_bounds(corners, 1))

		heights = np.array(get_bounds(corners, 2))[:, None] #all points height should be the same anyway
...
# 	data = get_corners()
# 	make_neighbors(data[0]["vertices"], data[0]["areas"], (-6, 6, -1, 11))